...
const Permission: React.FC = () => {
  ...
  const [createPermission] = useCustomMutation<
    {
      result: {
        created: boolean;
        permissions: { permission: Pick<Permission, "name"> }[];
      };
    },
    {
      input: {
        username: string;
        data: { resource: Resource; levels: Level[] }[];
      };
    }
  >(CREATE_PERMISSION_FOR_USER_MUTATION, {
    onSuccess({ result }) {
      if (result.created) {
    ...
    onError(error) {
      customSwalError(
        error,
        `Ocurrio un error al intentar otorgar permisos al usuario (${username})`,
      );
    },
  });
  ...
  const submit = (data: FormValues) => {
    createPermission({
      input: { username, data: Object.values(data.permissions) },
    });
    setShow(false);
    reset();
  };
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>
            Crear permisos al usuario: <strong>{state.username}</strong>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form
