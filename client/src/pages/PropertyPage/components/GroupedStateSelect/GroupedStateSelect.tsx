...
const GroupedStateSelect: React.FC<GroupedStateSelectProps> = ({}) => {
  ...
  const { items: groupedStates, setItems: setGroupedStates } =
    useGroupedStateStore();
  const { mutationDelete: mutationGroupedStateDelete } =
    useGroupedStateMutations<{ groupedState: GroupedState }>();
  const { error } = useCustomQuery<{ groupedStates: GroupedState[] }>(
    GET_ALL_GROUPED_STATES_QUERY,
    ["getAllGroupedStates"],
    {
      ...
      <Controller
        ...
        render={({ field }) => (
          <SelectNameable
            ...
            onDelete={() => {
              const groupedState = getValues("groupedState");
