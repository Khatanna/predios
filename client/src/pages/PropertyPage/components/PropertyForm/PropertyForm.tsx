...
const DynamicInput: React.FC<
  ...
  } & Omit<ControllerProps<Property>, "render">
> = ({
  disabled = false,
  name,
  title,
      ...
      render={({ field }) =>
        showEdit ? (
          ...
          <InputGroup>
            ...
            >
              <option disabled selected={!showEdit}>
                {placeholder}
              </option>
...
const PropertyForm: React.FC<PropertyFormProps> = ({ property }) => {
  ...
  const { register, handleSubmit, control, setValue } = useForm<Property>({
  ...
  const { data, error } = useCustomQuery<ResponseAPI>(GET_QUERY, [
    "getFieldForCreate",
  ]);
  ...
  const [createCity] = useCustomMutation<{ result: City }, { name: string }>(
  ...
  const [updateCity] = useCustomMutation<
    { result: City },
  ...
  const [updateProvince] = useCustomMutation<
    { result: Province },
        ...
        <Row>
          <Col className="d-flex flex-column gap-3">
            ...
            <Row>
              ...
              <Col xs={2}>
                <Form.Group>
                  <Form.Label>Poligono</Form.Label>
                  <Form.Control
              ...
              <Col>
                <Form.Group>
                  ...
                  <DynamicInput
                    ...
                    onSelect={(value) => {
                      const province = city?.provinces.find(
                    ...
                    onAction={({ value }) => {}}
                  />
                  {/* {province && city!.provinces.length >= 1 && (
                      <InputGroup.Text>{province.code}</InputGroup.Text>
                    )} */}
                </Form.Group>
              </Col>
              <Col>
                      ...
                      <Controller
                        ...
                        render={() => (
                          <ListGroup.Item className="d-flex align-items-center gap-1">
                            <div className="me-auto">{field.name}</div>
                            <DropdownMenu>
