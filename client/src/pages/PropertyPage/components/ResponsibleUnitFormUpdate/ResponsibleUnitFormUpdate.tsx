...
const GET_RESPONSIBLE_UNIT_BY_NAME = `
  query GetResponsibleUnitByName($name: String) {
    unit: getUnit(name: $name) {
      name
    }
  }
`;
const ResponsibleUnitFormUpdate: React.FC<FormUpdateProps> = ({
  onHide,
  params,
}) => {
  const { data } = useCustomQuery<{ unit: ResponsibleUnit }>(
    GET_RESPONSIBLE_UNIT_BY_NAME,
    ["getResponsibleUnitByName", { name: params?.name }],
  );
  const { resetField } = useFormContext<Property>();
  const { register, handleSubmit } = useForm<ResponsibleUnit>({
    values: data?.unit,
  });
  const { mutationUpdate } = useResponsibleUnitMutations<{
