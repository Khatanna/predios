...
const Dots = ({ user }: { user: User }) => {
	...
	const CustomMenu = forwardRef(({ children, style, className, 'aria-labelledby': labeledBy }: any, ref: any) => {
		return <div
			ref={ref}
			style={style}
			className={className}
	...
	return <Dropdown >
		...
		<Dropdown.Menu as={CustomMenu} >
			<Dropdown.Item eventKey="1" onClick={() => navigate(`edit/${user.username}`)}>✏ Editar</Dropdown.Item>
			<Dropdown.Item eventKey="2" onClick={handleDeleteClick}>🗑 Eliminar</Dropdown.Item>
		</Dropdown.Menu>
	</Dropdown>
}
...
const UserList: React.FC<UserListProps> = ({ }) => {
	...
	if (error) {
		return <div>No hay usuarios {JSON.stringify(error.response?.data.errors[0].message)}</div>
	}
	...
	// 	highlightOnHover
	// 	pointerOnHover={true} responsive striped onRowClicked={(row) => {
