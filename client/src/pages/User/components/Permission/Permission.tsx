...
import { Accordion, Alert, Button, Col, Form, Modal, OverlayTrigger, Tooltip } from 'react-bootstrap';
...
const Permission: React.FC = () => {
	...
	const { data, isLoading, error } = useCustomQuery<{ data: { permissions: { status: string, createdAt: string, permission: Permission, user: User }[] } }, { username: string }>(GET_PERMISSION_BY_USERNAME, ['getPermissionByUsername'], { username });
			...
			actions={
				data?.data.permissions.length === (LocalResources.length * 4) ? null :
					<OverlayTrigger key={"left"} placement='left' overlay={
						...
						<PlusCircle size={32} color='orange' role='button' onClick={handleShow} data-bs-title="Crear un permiso para el usuario" />
					</OverlayTrigger>
			}
