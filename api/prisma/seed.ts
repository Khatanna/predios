...
function getData(resource: string) {
  return JSON.parse(fs.readFileSync(path.join(__dirname, `./seeds/${resource}.json`), {
    encoding: 'utf-8'
  }))
}
...
async function main() {
  ...
  await prisma.user.createMany({
    data: users.map(({ names, firstLastName, secondLastName, username, password, typeId }) => ({ names, firstLastName, secondLastName, username, password, typeId }))
  })
  ...
  await prisma.user.create({
    data: {
      ...
      permissions: {
        connect: permissions.map(({ resource, level }) => {
          return {
            resource_level: {
              resource,
              level
