...
async function main() {
  ...
  const trackingsMapped = trackings.map(({ codigo_de_predio, fecha_de_inicio, observacion, nota, estado, responsable }) => {
    const stateName = (+estado === 27) ? 'Con Proyecto de Resolucion elaborado' :
      +estado === 23 ? 'RFS remitida a Control de Calidad' :
        +estado === 28 ? 'Elaboracion de Proyecto de Resolucion' :
          +estado === 44 ? 'Paralizado/Observado' :
            +estado === 47 ? 'Control de Calidad Relevamiento en campo' :
              +estado === 58 ? 'SOLICITUD ADMITIDA' :
                +estado === 61 ? 'RELEVAMIENTO DE INFORMACION EN CAMPO TERMINADO' :
                  +estado === 65 ? 'CON PROYECTO DE RESOLUCION FINAL DE SANEAMIENTO' :
                    +estado === 66 ? 'FALTA DE DOCUMENTOS' :
                      +estado === 67 ? 'PARALIZADA POR OPOSICION AL SANEAMIENTO' :
                        +estado === 71 ? 'POR REPLANTEO' :
                          +estado === 72 ? 'CON SENTENCIA DE TRIBUNAL AGROAMBIENTAL' :
                            +estado === 80 ? 'REMITIDA A OTRAS INSTITUCIONES' :
                              +estado === 81 ? 'CONFLICTO LIMITE POLITICO ADMINISTRATIVO' :
                                +estado === 82 ? 'CONFLICTO ORGANICO' :
                                  +estado === 83 ? 'COMPLEMENTACION EN CAMPO' :
                                    +estado === 84 ? 'CONFLICTO DE DERECHO PROPIETARIO' :
                                      +estado === 85 ? 'CONFLICTO DE LIMITES ENTRE PREDIOS' :
                                        +estado === 128 ? 'Conflicto' :
                                          pivot!.name

    const username = +responsable === 1 ? 'constancia.rodriguez' :
      +responsable === 2 ? 'miriam.castañeta' :
        +responsable === 3 ? 'milenka.escobar' :
          +responsable === 4 ? 'mayra.ramirez' :
            +responsable === 5 ? 'lizeth.castillo' :
              +responsable === 6 ? 'cristina.centellas' :
                +responsable === 7 ? 'andrea.villarroel' :
                  +responsable === 8 ? 'javier.herrera' :
                    +responsable === 9 ? 'carmen.acapa' :
                      +responsable === 10 ? 'miriam.callisaya' :
                        +responsable === 12 ? 'zulema.mollinedo' :
                          +responsable === 13 ? 'nelson.alcon' :
                            +responsable === 14 ? 'franz.isla' :
                              +responsable === 15 ? 'lisbeth.soliz' :
                                +responsable === 16 ? 'denisse.gabriel' :
                                  +responsable === 17 ? 'abraham.luna' :
                                    +responsable === 18 ? 'regina.tarqui' :
                                      +responsable === 20 ? 'delmira.endara' :
                                        +responsable === 21 ? 'benito.flores' :
                                          +responsable === 22 ? 'omar.mamani' :
                                            +responsable === 23 ? 'luis.canaviri' : undefined;
    if (propertyIDS[codigo_de_predio]) {
      return prisma.tracking.create({
        data: {
          dateOfInit: fecha_de_inicio ? fecha_de_inicio.replace(/\//g, '-').split('-').reverse().join('-') : '12/12/2023',
