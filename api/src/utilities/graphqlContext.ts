...
import { PrismaClient, User, LevelPermission, Resource } from "@prisma/client";
...
const createRecord = async (
  { model, args, operation, query }: any,
  username: string,
  ip?: string,
) => {
...
export const graphqlContext = async ({
  ...
  if (token) {
    const user = getUserWithAccessToken(token) as Pick<
      User,
      "username" | "status"
    >;
    if (!user) {
    ...
    return {
      prisma: prisma.$extends({
        query: {
          $allModels: {
            create: (options) =>
              createRecord(options, user.username, req.socket.remoteAddress),
            createMany: (options) =>
              createRecord(options, user.username, req.socket.remoteAddress),
            update: (options) =>
              createRecord(options, user.username, req.socket.remoteAddress),
